...
| Excluded | Current targets without a generated MCP surface (Antigravity desktop, DSH, Pi) | — | Use Antigravity CLI for the desktop family; DSH is skills-only; Pi has no verified stdio config route. |
...
| Malformed skills crash the index | Pack has real defects (E-02) | Index-time **metadata** validation (skip missing-frontmatter, warn+truncate desc>1024, warn oversize body); body kept **raw** and returned as a clearly labeled indexed skill rather than corrupted by char-stripping |
| Indexed skill text tries to override task or safety constraints | Raw third-party pack content could be mistaken for authority | Label every payload as third-party content, not authority; server instructions require higher-priority instructions and safety constraints to win |
...
- **Tools** (≤4; **namespaced** to avoid host-flattened collisions with the native "skills" concept; model-controlled; `readOnlyHint`; `structuredContent`+text dual-return):
