...
export async function renderClaudeSkill(source) {
  return renderSkillMarkdown(source, {
    invocationPrefix: "/",
    generatedFor: "Claude Code",
    frontmatter: source.metadata.model_invocation ? [] : ["disable-model-invocation: true"],
    hostInstruction: "Claude Code exposes this Agent Skill as a slash command and may select it automatically only when model invocation is enabled.",
  });
}
...
export async function renderKimiCodeSkill(source) {
  ...
  return [
    ...
    "type: prompt",
    `disableModelInvocation: ${source.metadata.model_invocation ? "false" : "true"}`,
    "---",
    "",
    `# ${source.name}`,
    "",
    `Use explicit invocation: \`/skill:${source.name}\`.`,
    `This skill is generated by agent-surface from \`${source.relativePath}\` for Kimi Code.`,
    "",
    source.body,
  ].join("\n");
}
...
export function renderSkillMarkdown(source, options = {}) {
  ...
  const invocationInstruction = source.metadata.model_invocation
    ? "Use this skill when its description matches the task, or invoke it explicitly."
    : invocationPrefix === null
      ? "Use this skill only when the host or user selects it explicitly."
