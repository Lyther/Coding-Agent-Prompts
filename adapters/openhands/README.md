...
- OpenHands' SDK loads `~/.agents/skills` before `~/.openhands/skills`, so user-scope command and external skills use the shared AgentSkills root that Codex and Zed also consume. The shared SKILL.md wrapper is written to be valid in Codex, OpenHands, and Zed.
