...
2. **Git Setup**:
    - `git init` (if missing).
    - `.gitignore`: Add `dist/`, `*.log`, `.DS_Store`, `Thumbs.db`, `__pycache__/`, `.vscode/`, `.cursor/`, `.claude/`, `.gemini/`, `.agent/`, `.cursorrules`, `.geminirules`, `.roorules`, `.clinerules`, `.traerules`, `.windsurfrules`, `.stellarisrules`, `.cursorignore`, `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `*.code-workspace`.
    - **Remote Ban**: Project AI files are allowed for local modding workflow, but they must stay gitignored and never be committed or pushed.
...
2. **Generate `.cursor/mod-context.json`** (Gitignored): Store the prefix and paths:
    ...
    - This is local-only assistant context. Never commit it.
...
4. **Symlink Rules**:
    ...
    - Keep these rule files local-only and gitignored. They are not part of the remote mod source.
...
4. **LOCAL AI FILES ONLY**: `.cursor/`, `.cursor/mod-context.json`, `.cursorrules`, and any other assistant-specific overlays may exist locally, but must stay out of the remote repo.
