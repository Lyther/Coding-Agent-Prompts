...
- Required external skill packs are included in a full install. Packs marked optional are added only by `--category external`, which installs the complete native external set. Large `source-pack` catalogs (`anthropic-cybersecurity-skills`, `rev-skills`) are deliberately **not** emitted (no `skill_roots`) and are served just-in-time by Grimoire instead.
