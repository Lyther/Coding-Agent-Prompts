...
- A full install emits general assets. `--category external` adds optional general skill packs; `--category cybersecurity` adds the declared CTF/red-team packs and cybersecurity MCPs; `--category private` and `--category modding` remain explicit. Large `source-pack` catalogs (`anthropic-cybersecurity-skills`, `rev-skills`, `hack-skills`) are not emitted into startup skill roots and are served just-in-time by Grimoire instead. Grimoire stays one on-demand index over every registered source pack; categories control native files and MCP selection, not row-level visibility inside that index.
