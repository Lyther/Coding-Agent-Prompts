...
phase: arbitrate
description: "Prove command discovery, full-autonomy tool execution, and task-shaped materialization before assigning a CLI, extension, provider, or model to a workflow role."
---
...
Codex exposes this source command as an explicit Agent Skill. Invoke it as `$workflow-runtime`, not `/workflow-runtime`. A source file under `commands/` does not imply that every target has a native slash-command primitive.

`inspect` is the no-execution mode defined by the first phase gate. Use it when the caller asks to inspect, explain, or prove that this command loaded without running a runtime probe.

For every other phase, `--role`, an exact `--runtime`, and `--time-budget` are required. `materialization` and `full` also require `--filescope` and `--artifact`. Reject missing inputs instead of guessing `auto`, selecting the current host, or broadening the phase. Invocation arguments constrain this command: an explicit `inspect`, `reference-only`, `do not execute`, or `do not call tools` request always wins over the full-probe objective.
...
The runtime executing this command is the driver. The runtime named by `--runtime` is the candidate.
...
| Runtime | Generated surface | Explicit invocation |
|---|---|---|
| Codex | Agent Skill | `$<name>` |
| Claude Code | custom command | `/<group>:<name>` or the name shown by `/help` |
| Cline | workflow Markdown | `/<name>` |
| Kilo | workflow/custom command | `--command <name>` when headless support is confirmed |
| OpenCode | custom command | `--command <name>` or `/<name>` in the TUI |
| Goose | recipe | `--recipe <name-or-path>` |
| Antigravity CLI | plugin skill | invoke after `agy plugin validate` and `agy plugin enable` |

If a runtime can use the raw prompt but cannot discover or invoke its generated surface, mark the command/workflow capability `UNREACHABLE`. Do not silently fall back to pasting the command body and call the distribution successful.
...
Codex custom prompts are not the distribution mechanism. The compiler emits explicit-invocation skills, and current Codex resolves them with `$name`. Prefer `--output-schema` for role artifacts.
...
Cline CLI and the VS Code/Cursor/Windsurf extensions do not share one MCP settings file. Prove the route used by the selected host. Invoke a generated workflow as `/<name>`; including the `.md` suffix falls through as ordinary prompt text in current Cline CLI releases. A plain prompt proves the agent, not workflow discovery. Use a different runtime as the driver; do not run `cline` recursively from an active Cline workflow.
...
Use `--command <name>` only after the live runtime lists or resolves the generated command. Provider credentials are runtime-specific; an extension login does not prove the headless CLI.
...
Any required capability set to false makes the runtime ineligible for that role. `prompt-only` cannot prove a generated command/workflow claim. Unknown model identity degrades independence claims even when the runtime is operational.

## VERDICT

- `ELIGIBLE`: the exact orchestration invocation, generated surface, full-autonomy mode, world state, and output contract passed.
- `PARTIAL`: real tool execution passed, but command discovery, exact output, liveness, or MCP reachability failed.
...
Do not replace a blocked real runtime with a mock, fixture, fake provider, copied command body, or direct MCP call and report it as passed.
...
- Do not call Codex command sources native workflows; they compile to Agent Skills.
- Do not give a read-only role write tools merely because host execution consent is full.
- Do not launch the candidate from the same runtime family that is executing this command.
